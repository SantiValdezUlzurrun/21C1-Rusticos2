...
mod tests {
    ...
    use crate::BaseDeDatos::TipoRedis;
    ...
    fn copy_copia_el_valor_de_una_clave_en_otra(){
    	...
    	let comando = vec!["copy".to_string(),"clave".to_string(),"otra_clave".to_string()];
    	copy(&comando,ptr_arc);

    	assert_eq!(arc_clone.lock().unwrap().obtener_valor("otra_clave").unwrap(), TipoRedis::Str("valor".to_string()));
    }
    ...
    fn rename_cambia_modifica_la_clave_pedida(){
        ...
        let comando = vec!["rename".to_string(),"clave".to_string(),"otra_clave".to_string()];
    ...
    fn tipo_devuelve_el_tipo_del_valor_almacenado_con_esa_clave(){
        ...
        assert_eq!(tipo(&vec1,ptr1),ResultadoRedis::BulkStr("string".to_string()));
        assert_eq!(tipo(&vec2,ptr2),ResultadoRedis::BulkStr("lista".to_string()));
        assert_eq!(tipo(&vec3,ptr3),ResultadoRedis::BulkStr("set".to_string()));
        assert_eq!(tipo(&vec4,ptr4),ResultadoRedis::BulkStr("none".to_string()));
    }
}	
