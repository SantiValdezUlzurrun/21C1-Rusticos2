...
fn sadd(comando: &Vec<String>,  hash_map: Arc<Mutex<HashMap<String, TipoRedis>>>) -> ResultadoRedis {
    let (a_agregar, cantidad_ingresada) = match hash_map.lock().unwrap().get(&comando[1]) {
...
fn srem(comando: &Vec<String>,  hash_map: Arc<Mutex<HashMap<String, TipoRedis>>>) -> ResultadoRedis { 
    let (a_agregar, cantidad_eliminada) = match hash_map.lock().unwrap().get(&comando[1]) {
...
mod tests {
    ...
    fn sadd_cuando_se_envia_una_clave_que_no_esta_esta_se_crea_y_se_almacena_correctamente() {
        ...
        let vector = vec!["SADD".to_string(), "miClave".to_string(), "miValor".to_string()];
    ...
    fn sadd_cuando_se_envia_una_clave_invalida_se_envia_el_error_adecuado() {
        ...
        hash.insert("miClave".to_string(), TipoRedis::Str("unString".to_string()));
        let vector = vec!["SADD".to_string(), "miClave".to_string(), "miValor".to_string()];
        ...
        assert_eq!(
            ResultadoRedis::Error("WrongTypeError error al obtener el set, valor guardado en la clave no es un Set".to_string()), 
            resultado, 
        );
    }
    ...
    fn sismember_cuando_se_envia_una_clave_que_no_esta_devuelve_0_ya_que_no_pertenece() {
        let hash: HashMap<String, TipoRedis> = HashMap::new();  
        let vector = vec!["SISMEMBER".to_string(), "miClave".to_string(), "miValor".to_string()];
    ...
    fn srem_cuando_se_envia_una_clave_que_no_esta_no_se_elimina_ningun_valor() {
        let hash: HashMap<String, TipoRedis> = HashMap::new(); 
        let vector = vec!["SREM".to_string(), "miClave".to_string(), "miValor".to_string()];
    ...
    fn srem_cuando_se_envia_una_clave_que_posee_dos_elementos_y_se_eliminan_solo_uno_dos_veces_no_se_encuentra_el_restante() {
        ...
        hash.insert("miClave".to_string(), TipoRedis::Set(set));
        let vector = vec!["SREM".to_string(), "miClave".to_string(), "miValor".to_string(), "miValor".to_string()];
