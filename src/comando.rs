...
pub trait Command {
	fn ejecutar(&self, hash_map:&mut HashMap<String, String>) -> Result<ResultadoRedis, CommandError>;
}
...
pub fn crear_comando(arg_vec: &Vec<String>) -> Box<dyn Command>{	

	if "SET".to_string() == arg_vec[0]{
		Box::new(SetCommand::new(arg_vec[1].to_string(),arg_vec[2].to_string()))
	}else {
...
mod tests {
    ...
    fn fabrica_crea_un_set_comand_guardo_un_valor(){
        ...
        let arg_vec = vec!("SET".to_string(),"una_clave".to_string(),"un_valor".to_string());
    ...
    fn fabrica_crea_set_comand_guardo_dos_valores_con_la_misma_clave(){
        ...
        //primer comando get//
        let arg_vec1 = vec!("SET".to_string(),"una_clave".to_string(),"un_valor".to_string());
        let set1 = crear_comando(&arg_vec1);
        set1.ejecutar(&mut hash_map).unwrap();

        //segundo comando get//
        let arg_vec2 = vec!("SET".to_string(),"una_clave".to_string(),"otro_valor".to_string());
        let set2 = crear_comando(&arg_vec2);
        set2.ejecutar(&mut hash_map).unwrap();


        assert_eq!(hash_map.get(&"una_clave".to_string()).unwrap(), &"otro_valor".to_string());
	}
